...
class PretrainingModel(object):
  ...
  def __init__(self, config: configure_pretraining.PretrainingConfig,
    ...
    # Generator
    # 使用bert的embedding，然后通过bi-gru和softmax对输入的每个token进行正确与错误的判断
    embedding_size = (
        self._bert_config.hidden_size if config.embedding_size is None else
        config.embedding_size)
    generator = self._build_bigru(pretrain_data.features_to_inputs(features).input_ids)
    gene_output = self._get_detect_output(pretrain_data.features_to_inputs(features).input_ids, generator)
    self.mlm_output = gene_output
    self.total_loss = gene_output.loss
  ...
  def _build_bigru(self, inputs: pretrain_data.Inputs, is_training,
                         bert_config=None, name="electra", reuse=False, **kwargs):
    """Build a bigru network."""
    # with tf.variable_scope(tf.get_variable_scope(), reuse=reuse):
    with tf.variable_scope(tf.get_variable_scope(), reuse=reuse):
        return modeling.bigru(input_ids=inputs.input_ids)
  ...
  def _get_detect_output(self, inputs: pretrain_data.Inputs, model):
    """bi-gru modeling softmax layer."""
    # with tf.variable_scope("hidden"):
    #     w = tf.get_variable("W", shape=[256, 128], dtype=tf.float32)
    #     b = tf.get_variable("b", shape=[128], dtype=tf.float32)
    #
    #     output = tf.reshape(gru_output, shape=[-1, 256])
    #     hidden = tf.tanh(tf.nn.xw_plus_b)
    #
    # with tf.variable_scope("logits"):
    #     w = tf.get_variable("W", shape=[128, 2], dtype=tf.float32)
    #     b = tf.get_variable("b", shape=[2], dtype=tf.float32)
    #     pred = tf.nn.xw_plus_b(hidden, w, b)
    #     self.logits = tf.reshape(pred, shape=[-1, 128, 2])
    ...
    #         model.get_sequence_output(), inputs.masked_lm_positions)
        ...
        output_bias = tf.get_variable(
            ...
            shape=[2],
            initializer=tf.zeros_initializer())
        logits = tf.matmul(hidden, 128,
                           transpose_b=True)
        logits = tf.nn.bias_add(logits, output_bias)
      oh_labels = tf.one_hot(
          inputs.label_id, depth=2,
          dtype=tf.float32)
      probs = tf.nn.softmax(logits)
      log_probs = tf.nn.log_softmax(logits)
      label_log_probs = -tf.reduce_sum(log_probs * oh_labels, axis=-1)
      ...
      numerator = tf.reduce_sum(inputs.masked_lm_weights * label_log_probs)
      denominator = tf.reduce_sum(inputs.masked_lm_weights) + 1e-6
      loss = numerator / denominator
      preds = tf.argmax(log_probs, axis=-1, output_type=tf.int32)
