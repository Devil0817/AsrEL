...
class FinetuningModel(object):
  ...
  def __init__(self, config: configure_finetuning.FinetuningConfig, tasks,
    ...
    self.embedding = bert_model.get_pooled_output()
...
class ModelRunner(object):
  ...
  def evaluate_task(self, task, split="dev", return_results=True):
    ...
    # print("result len---1: ", sum(1 for _ in results))
    if task.name == "cmrc2018" or task.name == "drcd":
      scorer = task.get_scorer(split)
    elif task.name == 'snips':
      scorer = task.get_scorer()
    else:
      scorer = task.get_scorer(split)
    # print("result len---2: ", sum(1 for _ in results))
    for r in results:
  ...
  def write_classification_outputs(self, tasks, trial, split):
    ...
    # print("result len: ", len(list(results)))
    ...
    for r in results:
      if r["task_id"] != len(self._tasks):
        ...
        embedding.append([r[task_name]['inputid'], r[task_name]['logits'], r[task_name]['cls']])
    ...
    # 将embedding写入文件
    # embedding_file = '/home/huqian/Chinese-ELECTRA/embedding_file.txt'
    # with open(embedding_file, 'w', encoding='utf8') as wf:
      ...
      if config.write_test_outputs and trial <= config.n_writes_test:
        ...
        for task in tasks:
          ...
          if task.name in ["cola", "mrpc", "mnli", "sst", "rte", "qnli", "qqp",
                           "sts", "sps", "afqmc", "cmcc", "atis", "dstc", "chunk"]:
            if task.name == "afqmc" or task.name == 'sst':
